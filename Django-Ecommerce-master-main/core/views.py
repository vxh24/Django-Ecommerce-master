...
class ItemDetailView(View):
    def get(self, *args, **kwargs):
        select_item = Item.objects.get(slug=self.kwargs['slug'], is_active=True)
        review_list = []
        reviews = Review.objects.filter(item = select_item.id)
        form = ReviewForm()
...
def add_to_cart(request, slug):
    ...
    #control stock_no
    current_stock =int(item.stock_no)
    #Control quantity
